...
def export_progress_by_date_range(repo, updates, days):
...
def export_daily_progress(repo, updates):
    # 构建仓库的日志文件目录
    repo_dir = os.path.join('daily_progress', repo.replace("/", "_"))
...
class ReportGenerator:
    ...
    def generate_daily_report(self, markdown_file_path, prompt=None):
        # 读取Markdown文件并使用LLM生成日报
        with open(markdown_file_path, 'r') as file:
            markdown_content = file.read()

        report = self.llm.generate_daily_report(markdown_content, prompt=prompt)  # 调用LLM生成报告
    ...
    def generate_report_by_date_range(self, markdown_file_path, days, prompt=None):
        # 生成特定日期范围的报告，流程与日报生成类似
        with open(markdown_file_path, 'r') as file:
            markdown_content = file.read()

        report = self.llm.generate_daily_report(markdown_content, prompt=prompt)

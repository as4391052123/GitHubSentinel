...
class ReportGenerator:
    ...
    def generate_github_daily_report(self, markdown_file_path):
        ...
        report = self.llm.generate_github_daily_report(markdown_content)  # 调用LLM生成报告
    ...
    def generate_hn_daily_report(self, markdown_file_path):
        # 读取Markdown文件并使用LLM生成日报
        with open(markdown_file_path, 'r') as file:
            markdown_content = file.read()

        report = self.llm.generate_hn_daily_report(markdown_content)  # 调用LLM生成报告
    ...
    def generate_report_by_date_range(self, markdown_file_path, days):
        ...
        report = self.llm.generate_github_daily_report(markdown_content)
    ...
    def generate_hn_topic_report(self, markdown_file_path):
        ...
        report = self.llm.generate_hn_daily_report(markdown_content)

        report_file_path = os.path.splitext(markdown_file_path)[0] + f"_report.md"
        with open(report_file_path, 'w+') as report_file:
            report_file.write(report)

        LOG.info(f"Hacker News 前沿技术趋势已保存到 {report_file_path}")
